...
function startNode() {
    ...
    app.get('/', function(req, res) {
        ...
        else if (typeof pushtx !== undefined && getData.pushtx) {
            var from = sha256('' + getData.pkey).substring(0, 10);
            var stmt = db.prepare("SELECT * FROM addresses WHERE address = ?");
            stmt.get(from, function(err, row) {
                if (err) {
                    throw(err);
                } else {
                    if (row.balance < getData.amt) {
                        res.send("Error1");
                        res.end();
                    }
                }
